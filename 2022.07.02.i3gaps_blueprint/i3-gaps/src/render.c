...
int render_deco_height(void) {
    int deco_height = config.font.height + 8 + 8;
...
static void render_con_tabbed(Con *con, Con *child, render_params *p, int i) {
    ...
    child->deco_rect.width = floor((float)(child->rect.width-18) / p->children);
    child->deco_rect.x = 8+ p->x - con->rect.x + i * child->deco_rect.width;
    ...
    if (i == (p->children - 1)) {
        child->deco_rect.width = child->rect.width - child->deco_rect.x - 8;
    }
