...
static void x_draw_title_border(Con *con, struct deco_render_params *p) {
    ...
    draw_util_rectangle(&(con->parent->frame_buffer), p->color->border,
                        dr->x, dr->y, 2, dr->height+1);

    // left touchup
    draw_util_rectangle(&(con->parent->frame_buffer), config.client.background,
                        0, 0, 8, 50);

    /* Right */
    draw_util_rectangle(&(con->parent->frame_buffer), p->color->border,
                        dr->x + dr->width - 2, dr->y, 2, dr->height+1);
    /* Right touchup */
    draw_util_rectangle(&(con->parent->frame_buffer), config.client.background,
                        con->window_rect.width+12, 0, 8, 50);

    /* Top Clear */
    draw_util_rectangle(&(con->parent->frame_buffer), config.client.background,
                        dr->x, dr->y, dr->width, 8);
    /* Top */
    draw_util_rectangle(&(con->parent->frame_buffer), p->color->border,
                        dr->x, dr->y+8, dr->width, 2);

    /* Bottom */
    draw_util_rectangle(&(con->parent->frame_buffer), p->color->border,
                        dr->x+2, dr->y + dr->height - 2, dr->width-4, 2);
}
...
void x_draw_decoration(Con *con) {
	...
	// 1st border = spacer
	const int second_border = 2; // window outline
	const int third_border = 8; // active window 
        for (size_t i = 0; i < rectangles_count; i++) {
            draw_util_rectangle(&(con->frame_buffer), config.client.background,
                                ...
                                rectangles[i].width,
                                rectangles[i].height);
            //if(con_num_visible_children(con_get_workspace(con)) <= 1) {
	    //	continue;
	    //	}
	    // x=0, y=0 -> left
	    if(rectangles[i].x == 0 && rectangles[i].y == 0) {
		draw_util_rectangle(&(con->frame_buffer), p->color->child_border,
					rectangles[i].x + rectangles[i].width - second_border,
					rectangles[i].y + (rectangles_count-3)*rectangles[i].width,
					second_border,
					rectangles[i].height - (rectangles_count-2)*rectangles[i].width);
	    	continue;
	    }
	    // x>0, y>0 -> bottom
	    if(rectangles[i].x > 0 && rectangles[i].y > 0) {
		draw_util_rectangle(&(con->frame_buffer), p->color->child_border,
					rectangles[i].x-second_border,
					rectangles[i].y,
					rectangles[i].width+2*second_border,
					second_border);
		draw_util_rectangle(&(con->frame_buffer), p->color->indicator,
					rectangles[i].x+third_border,
					rectangles[i].y+second_border,
					rectangles[i].width,
					third_border);
	    	continue;
	    }
	    // y=0, w<0 -> right
	    if(rectangles[i].y == 0 && rectangles[i].width < rectangles[i].height) {
            	draw_util_rectangle(&(con->frame_buffer), p->color->child_border,
                                rectangles[i].x,
                                rectangles[i].y + (rectangles_count-3)*rectangles[i].width,
                                second_border,
            	...
            	draw_util_rectangle(&(con->frame_buffer), p->color->indicator,
                                rectangles[i].x+second_border,
                                rectangles[i].y + (rectangles_count-3)*rectangles[i].width +third_border+second_border,
                                third_border,
                                rectangles[i].height - (rectangles_count-2)*rectangles[i].width);
	    	continue;
	    }
	    ...
	    if(rectangles[i].y == 0 && rectangles[i].width > rectangles[i].height) {
	        draw_util_rectangle(&(con->frame_buffer), p->color->child_border,
					rectangles[i].x-second_border,
					rectangles[i].y+rectangles[i].height-second_border,
					rectangles[i].width+2*second_border,
					second_border);
	    	continue;
	    }
	...
	*/
    ...
    /* 6: draw the title */
    int text_offset_y = (con->deco_rect.height - config.font.height + 8) / 2;
